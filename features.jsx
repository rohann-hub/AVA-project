...
/* ------------------------------------------------------------------
   Arc lookup table — normalised, so amplitude is applied at runtime.
   Sweep runs 60deg -> 120deg:
     x:  +1  ->  0  ->  -1   (enters right, exits left)
     y:   1  ->  0  ->   1   (dips low at the edges, flat at centre)
     rot:+30 ->  0  -> -30   (upright exactly at centre)
   At the midpoint of a card's window the card is dead-centre and
   perfectly upright. That part of the original math was already right.
------------------------------------------------------------------- */
const ARC_STEPS = 40;
const ARC_X = [];
const ARC_Y = [];
...
for (let i = 0; i <= ARC_STEPS; i++) {
  const angle = 60 + 60 * (i / ARC_STEPS);
  const rad = (angle * Math.PI) / 180;
  ARC_X.push(Math.cos(rad) / 0.5);                    // -> [-1, 1]
  ARC_Y.push((1 - Math.sin(rad)) / (1 - Math.sin(Math.PI / 3))); // -> [0, 1]
  ARC_R.push(90 - angle);
}

/* Scroll windows -------------------------------------------------- */
const CARD_SPAN = 0.18;   // how long one card stays on screen
const CARD_STEP = 0.072;  // gap between consecutive card entries
const CARD_START = 0.02;

// A caption may only be visible for less than one CARD_STEP, otherwise two
// consecutive captions are on screen at the same time and print over
// each other. 0.48 leaves a small dead zone between them.
const CAPTION_HALF = CARD_STEP * 0.48;
const CAPTION_HOLD = 0.012; // fully-opaque plateau at the centre
...
const NEIGHBOUR_SCALE = 0.84; // how small a card is at the edge of its window
...
export default function FeaturesReveal() {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      // Arc must be wide enough that a neighbour clears the centre card.
      const ampX = Math.max(cardW * 1.55, Math.min(1150, vw * 0.6));
      // Shallow dip — a card rotated 12deg is already ~110px taller than
      // its upright bounding box, which is what was landing on the caption.
      const ampY = Math.round(Math.min(80, vh * 0.1));
    ...
    const applyFrame = (raw) => {
      ...
      for (let t = 0; t < featuresData.length; t++) {
        ...
        const winStart = CARD_START + CARD_STEP * t;
        const winMid = winStart + CARD_SPAN / 2;

        const rel = clamp01((progress - winStart) / CARD_SPAN);

        // 1 when the card is dead-centre, 0 at either end of its window
        const centreness = 1 - Math.abs(rel - 0.5) * 2;

        // Interpolate along the arc
        const fi = rel * ARC_STEPS;
        const i0 = Math.floor(fi);
        const i1 = Math.min(ARC_STEPS, i0 + 1);
        const w = fi - i0;

        const nx = ARC_X[i0] + w * (ARC_X[i1] - ARC_X[i0]);
        const ny = ARC_Y[i0] + w * (ARC_Y[i1] - ARC_Y[i0]);
        const rot = ARC_R[i0] + w * (ARC_R[i1] - ARC_R[i0]);

        const x = nx * L.ampX;
        const y = ny * L.ampY + L.yOffset;

        // Depth cue: only the centre card is at full size and full opacity,
        // so neighbours read as background instead of competing for attention.
        const scale = NEIGHBOUR_SCALE + (1 - NEIGHBOUR_SCALE) * centreness;
        const cardOpacity = clamp01(centreness / 0.8);

        // Caption: narrow window, so exactly one is ever on screen
        let textOpacity = 0;
        const dist = Math.abs(progress - winMid);
        if (dist <= CAPTION_HALF) {
          textOpacity =
            dist <= CAPTION_HOLD
              ? 1
              : (CAPTION_HALF - dist) / (CAPTION_HALF - CAPTION_HOLD);
        }

        // Nearest-to-centre card always wins the stack
        const z = 1 + Math.round(centreness * 100);

        el.style.opacity = String(cardOpacity);
        el.style.zIndex = String(z);
        el.style.transform = `translate3d(${x}px, ${y}px, 0) rotate(${rot}deg) scale(${scale})`;
        el.style.pointerEvents = cardOpacity > 0.9 ? 'auto' : 'none';
